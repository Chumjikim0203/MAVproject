					...
					<c:if test="${room.roomNum == MatchRoom.roomNum}">
						<div class="card-1 mb-4">
							<div class="room-info">
								...
								<div class="room-title room-dt">
									<span class="romm-ddt">no.${MatchRoom.roomNum}
										&nbsp${MatchRoom.roomName} </span><br> <span class="romm-ddt">${MatchRoom.matchTitle}</span>
								</div>
								<span class="room-application "> <a
									href="<c:url value='/match/matchingDetail'/>?roomNum=${MatchRoom.roomNum}"
									class="room-application-link room-application-btn2">조회</a> <!-- 수정코드 필요여부에 따라 -->
									<%-- <a href="<c:url value='/room/roomsUpdate'/>?roomNum=${room.roomNum}" class="room-application-link room-application-btn2">수정</a> --%>
									<a href="#" class="room-application-link room-application-btn2"
									onclick="confirmDelete(${room.roomNum});">삭제</a>
								</span>
