...
public class NoticeRepositoryImpl implements NoticeRepository {
    ...
    public void noticeWrite(String noticeWritor, String title, String content, LocalDate now) {
        ...
        jdbcTemplate.update(sql, noticeWritor, title, content, now);
    ...
    public void updateNotice(Notice updateNotice) {
        ...
        jdbcTemplate.update(sql, updateNotice.getNoticeTitle(), updateNotice.getNoticeContents(),
    ...
    public Notice getPreviousNotice(int noticeNum) {
        String sql = "SELECT * FROM notice WHERE noticeNum < ? ORDER BY noticeNum DESC LIMIT 1";
        try {
            return jdbcTemplate.queryForObject(sql, new Object[] { noticeNum }, new NoticeRowMapper());
    ...
    public Notice getNextNotice(int noticeNum) {
        String sql = "SELECT * FROM notice WHERE noticeNum > ? ORDER BY noticeNum ASC LIMIT 1";
        try {
            return jdbcTemplate.queryForObject(sql, new Object[] { noticeNum }, new NoticeRowMapper());
        } catch (EmptyResultDataAccessException e) {
            return null;
