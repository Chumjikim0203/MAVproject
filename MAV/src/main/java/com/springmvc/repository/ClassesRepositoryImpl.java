...
// 값넣기 컬럼명순서
	...
	{
		classes.setClassApprove(true);
	    String SQL= "insert into classes(classId,className,classLectureDate,classLecturetime,classNumberOfStudents,classCourseContents,classGrade,classIntroduction,classImages,classLocale,classApprove) Values(?,?,?,?,?,?,?,?,?,?,?)";
	    System.out.println("setnewCLASS실행");
	    template.update(SQL, classes.getteacherId(), classes.getClassName(), classes.getClassLecturedate(), 
	    					 classes.getClassLecturetime(), classes.getClassNumberOfStudents(), classes.getClassCourseContents(),
	    					 classes.getClassGrade(), classes.getClassIntroduction(), classes.getClassImages(), 
	    					 classes.getClassLocale(), classes.isClassApprove());
	}
